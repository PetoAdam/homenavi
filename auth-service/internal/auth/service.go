...
func NewService(cfg Config) *Service {
	...
	return &Service{config: cfg, redisClient: redisClient, googleOAuthConfig: googleOAuthConfig}
}
...
func (s *Service) IssueAccessToken(user *entities.User) (string, error) {
...
func (s *Service) IssueRefreshToken(userID string) (string, error) {
	...
	if err := s.redisClient.Set(ctx, "refresh_token:"+tokenID, userID, s.config.RefreshTokenTTL).Err(); err != nil {
		return "", err
	}
	return tokenID, nil
}
...
func (s *Service) ValidateRefreshToken(token string) (string, error) {
	ctx := context.Background()
	userID, err := s.redisClient.Get(ctx, "refresh_token:"+token).Result()
	if err != nil {
...
func (s *Service) IsLoginLocked(email string) (bool, int64, error) {
	ctx := context.Background()
	ttl, err := s.redisClient.TTL(ctx, "login_lockout:"+email).Result()
...
func (s *Service) RegisterLoginFailure(email string) (bool, int64, error) {
	...
	s.redisClient.Expire(ctx, failKey, time.Duration(s.config.LoginLockoutSeconds)*time.Second)
	if int(count) >= s.config.LoginMaxFailures {
		_ = s.redisClient.Set(ctx, "login_lockout:"+email, "1", time.Duration(s.config.LoginLockoutSeconds)*time.Second).Err()
...
func (s *Service) IsCodeLocked(userID, codeType string) (bool, int64, error) {
	ctx := context.Background()
	ttl, err := s.redisClient.TTL(ctx, "code_lockout:"+userID+":"+codeType).Result()
	if err != nil && err != redis.Nil {
		return false, 0, err
	}
	if ttl > 0 {
...
func (s *Service) RegisterCodeFailure(userID, codeType string) (bool, int64, error) {
	...
	s.redisClient.Expire(ctx, failKey, time.Duration(s.config.CodeLockoutSeconds)*time.Second)
	...
	if int(count) >= s.config.CodeMaxFailures {
		...
		_ = s.redisClient.Set(ctx, lockKey, "1", time.Duration(s.config.CodeLockoutSeconds)*time.Second).Err()
...
func (s *Service) GenerateOAuthState() (string, error) {
	...
	ctx := context.Background()
	if err := s.redisClient.Set(ctx, "oauth_state:"+state, "valid", 10*time.Minute).Err(); err != nil {
...
func (s *Service) ValidateOAuthState(state string) error {
	ctx := context.Background()
	if s.redisClient.GetDel(ctx, "oauth_state:"+state).Err() != nil {
...
func (s *Service) ExchangeGoogleOAuthCode(code, redirectURI string) (*GoogleUserInfo, error) {
...
func (s *Service) HandleGoogleCallback(code string) (*GoogleUserInfo, error) {
	ctx := context.Background()
