...
func (h *VerificationHandler) HandleEmailVerifyRequest(w http.ResponseWriter, r *http.Request) {
	var req emailtransport.EmailVerifyRequest
	...
	json.NewEncoder(w).Encode(emailtransport.VerificationResponse{
...
func (h *VerificationHandler) HandleEmailVerifyConfirm(w http.ResponseWriter, r *http.Request) {
	var req emailtransport.EmailVerifyConfirmRequest
	if err := sharedtransport.ParseAndValidateJSON(r, &req); err != nil {
		errors.WriteError(w, errors.BadRequest(err.Error()))
		return
	}
	...
	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(emailtransport.SuccessResponse{
