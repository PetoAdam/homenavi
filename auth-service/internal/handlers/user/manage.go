...
func (h *ManageHandler) HandleList(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		errors.WriteError(w, errors.Unauthorized("invalid token claims"))
		return
	}
	if role != "resident" && role != "admin" {
		errors.WriteError(w, errors.Forbidden("forbidden"))
		return
	}

	users, meta, err := h.userService.ListUsers(r.URL.Query(), r.Header.Get("Authorization"))
	if err != nil {
		errors.WriteError(w, errors.InternalServerError("failed to list users", err))
		return
	}
...
func (h *ManageHandler) HandleGet(w http.ResponseWriter, r *http.Request) {
	...
	requester, ok := claimString(claims, "sub")
	if !ok {
		errors.WriteError(w, errors.Unauthorized("invalid token claims"))
		return
	}
	...
	if id == "" {
		errors.WriteError(w, errors.BadRequest("missing id"))
		return
	}
	if requester != id && role != "resident" && role != "admin" {
		errors.WriteError(w, errors.Forbidden("forbidden"))
		return
	}
...
func (h *ManageHandler) HandlePatch(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&updates); err != nil {
		errors.WriteError(w, errors.BadRequest("invalid body"))
		return
	}

	if newRoleRaw, ok := updates["role"]; ok {
		newRole, _ := newRoleRaw.(string)
		newRole = strings.ToLower(newRole)
		if role == "admin" {
			if newRole != "user" && newRole != "resident" && newRole != "admin" {
				errors.WriteError(w, errors.BadRequest("invalid role"))
				return
			}
		} else if role == "resident" {
			if newRole != "resident" {
				errors.WriteError(w, errors.Forbidden("residents can only grant resident"))
				return
			}
		} else {
			errors.WriteError(w, errors.Forbidden("cannot change role"))
			return
		}
	}
	if requester != id && role != "resident" && role != "admin" {
...
func (h *ManageHandler) HandleLockout(w http.ResponseWriter, r *http.Request) {
	...
	if role != "admin" {
		errors.WriteError(w, errors.Forbidden("admin only"))
		return
	}
	id := strings.TrimPrefix(r.URL.Path, "/api/auth/users/")
	if idx := strings.Index(id, "/"); idx != -1 {
	...
	bearer := r.Header.Get("Authorization")
	if err := h.userService.UpdateUser(id, updates, strings.TrimPrefix(bearer, "Bearer ")); err != nil {
		errors.WriteError(w, errors.InternalServerError("failed to update lockout", err))
		return
	}
	w.WriteHeader(http.StatusOK)
