...
func newTestServer(t *testing.T) *Server {
	t.Helper()
	dsn := "file:http_" + strings.NewReplacer("/", "_", " ", "_").Replace(t.Name()) + "?mode=memory&cache=shared"
...
func TestListStateReturnsPoints(t *testing.T) {
	...
	_ = s.repo.InsertStatePoint(context.Background(), &dbinfra.DeviceStatePoint{DeviceID: "zigbee/0x1", TS: received, Payload: []byte(`{"x":1}`)})
