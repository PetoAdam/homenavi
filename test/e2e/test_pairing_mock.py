...
PAIRING_PROGRESS_TOPIC = "homenavi/devicehub/events/pairing.progress"
...
def start_pairing(token: str, metadata: Dict[str, str]) -> PairingSession:
    resp = requests.post(
        f"{GATEWAY_ORIGIN}/api/devicehub/pairings",
        headers=_auth(token),
...
def create_mock_device(token: str, external_id: str) -> Dict[str, object]:
    payload = {
        "protocol": PAIRING_PROTOCOL,
        "external_id": external_id,
        "name": f"{PAIRING_DEVICE_NAME} {external_id[-4:]}",
        "type": "contact_sensor",
        "manufacturer": "MockCo",
        "model": "ZB-TEST",
    ...
    resp = requests.post(
        f"{GATEWAY_ORIGIN}/api/devicehub/devices",
        headers=_auth(token),
        cookies={"auth_token": token},
        json=payload,
        timeout=10,
...
def publish_pairing_progress(token: str, events: Iterable[Dict[str, object]]) -> None:
    ...
    try:
        ...
        while not connected:
            ...
            time.sleep(0.05)
        for evt in events:
            print("PAIRING_PROGRESS_EMIT", json.dumps(evt, sort_keys=True))
            data = json.dumps(evt, separators=(",", ":")).encode("utf-8")
            result = client.publish(PAIRING_PROGRESS_TOPIC, payload=data)
...
def delete_device(token: str, device_id: str, external_id: str) -> None:
    resp = requests.delete(
        f"{GATEWAY_ORIGIN}/api/devicehub/devices/{device_id}?force=1",
        headers=_auth(token),
        cookies={"auth_token": token},
        timeout=10,
    )
...
def run() -> None:
    ...
    token = login()
    print("LOGIN_OK")
    external_id = f"mock.zigbee.{uuid.uuid4().hex[:8]}"
    metadata = {
    ...
    session = start_pairing(token, metadata)
    print(f"PAIRING_START session_id={session.id} status={session.status} external_id={external_id}")
    device_record = None
    try:
        device_record = create_mock_device(token, external_id)
        device_id = device_record.get("id")
        print(f"MOCK_DEVICE_CREATED device_id={device_id} external_id={external_id}")
        progress_plan = [
            (
                {"protocol": PAIRING_PROTOCOL, "stage": "device_joined", "status": "joined", "external_id": external_id},
                ("device_joined",),
                "device_joined",
            ),
            (
                {"protocol": PAIRING_PROTOCOL, "stage": "interview_started", "status": "interviewing", "external_id": external_id},
                ("interviewing",),
                "interviewing",
            ),
            (
                {"protocol": PAIRING_PROTOCOL, "stage": "interview_complete", "status": "success", "external_id": external_id},
                ("interview_complete", "completed"),
                "interview_complete",
            ),
        ]
        for event, expected_statuses, label in progress_plan:
            publish_pairing_progress(token, [event])
        ...
        print(f"PAIRING_FINAL status={final_session.status} active={final_session.active}")
        if final_session.status != "completed":
            raise RuntimeError(f"unexpected final pairing status {final_session.status}")
        devices = list_devices(token)
        ...
        print(f"PAIRING MOCK TEST OK device_id={matched.get('id')} external_id={external_id}")
    finally:
        if not KEEP_DEVICE and device_record and device_record.get("id"):
            try:
                delete_device(token, device_record["id"], external_id)
                print(f"MOCK_DEVICE_REMOVED device_id={device_record['id']}")
            except Exception as exc:
                print(f"MOCK_DEVICE_REMOVE_FAILED {exc}")
